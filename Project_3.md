...
##### Another model validation technique used is root mean square error. Based on this, the linear regression model had greater error than the random forest model.
##### The random forests model underpredicted less than the linear model. Based on its high R^2 value and the plot of the differences between actual and predicted population sums as well as the root mean squared error, I would argue that the random forests model is the better of the two. It is likely that selecting variables more carefully and changing the number of trees used could further ipmprove the model as well. 
